...
export default function CopyButton({ text, className = "" }: CopyButtonProps) {
  ...
  const [notification, setNotification] = useState<{ message: string; subMessage: string; type: 'email' | 'phone' | null }>({ 
    message: '', 
    subMessage: '', 
    type: null 
  });
  ...
  const handleCopyToClipboard = (textToCopy: string) => {
    navigator.clipboard.writeText(textToCopy).then(() => {
      ...
      setNotification({
        ...
        subMessage: isEmail ? `${textToCopy} copied successfully.` : 'Text copied successfully.',
  ...
  return (
    <>
      <span 
        className={`copyable group cursor-pointer inline-flex items-center gap-1 ${className}`}
      ...
      >
        ...
        <svg 
          ...
          className="inline-block text-zinc-400 group-hover:text-zinc-100 transition-colors"
        >
          <path 
      ...
      {notification.message && (
        <div className="fixed top-4 right-4 z-50 bg-green-600 text-white px-4 py-2 rounded-lg shadow-lg">
          ...
          <div className="text-sm opacity-90">{notification.subMessage}</div>
        </div>
      )}
