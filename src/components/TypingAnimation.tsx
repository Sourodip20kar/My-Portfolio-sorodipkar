...
export default function TypingAnimation({ text, scrollRef, onCompleted, speed = 10 }: TypingAnimationProps) {
  ...
  const parseOutput = (output: string) => {
    ...
    // This regex now correctly handles multiple copy blocks during animation
    processedText = processedText.replace(/COPY\{([^}]+)\}/g, (match, content) => {
        return `<span class="copyable group cursor-pointer" data-copy="${content}">${content}${copyIconHtml}</span>`;
    });

    // This handles the case where the typing animation is halfway through a COPY block
    processedText = processedText.replace(/COPY\{([^}]+)$/, (match, content) => {
        return `<span class="copyable group cursor-pointer" data-copy="${content}">${content}</span>`;
    });
  ...
  return (
    <div 
      dangerouslySetInnerHTML={{
        __html: parseOutput(displayedText)
      }}
    />
  );
};
